...
/**
 * @file Tool: fetches contacts from the connected CRM.
 *
...
const getContactsTool = {
  name: "get_contacts",
  description: "Fetches contacts from the connected CRM",

  async execute(input, context) {
    try {
      const adapter = integrationAdapterFactory.get("crm");
      const result = await adapter.getContacts(input.params);
...
export default getContactsTool;
