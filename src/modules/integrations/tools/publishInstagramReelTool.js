...
const publishInstagramReelTool = {
  name: "publish_instagram_reel",
  description:
    "Publishes an approved Instagram Reel via the connected Meta integration",
  ...
  async execute(input, context) {
    ...
    try {
      execution = await IntegrationExecution.create({
        ...
        operation: "publish_instagram_reel",
        toolName: "publish_instagram_reel",
        contentItemId,
        actorType: "agent",
    ...
    try {
      ...
      const result = await adapter.publishReel({ contentItemId });
...
export default publishInstagramReelTool;
